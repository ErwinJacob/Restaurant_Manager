//
 ...
 class Takings: Identifiable{
     ...
     init(takings: String) {
         ...
         let currentTime = Date() //get current time
         
         let dateFormatter = DateFormatter()
         dateFormatter.dateFormat = "HH:mm"

         self.time = dateFormatter.string(from: currentTime)

         dateFormatter.dateFormat = "dd-MM-yyyy"

         self.id = dateFormatter.string(from: currentTime) + "-" + self.time
     }

     func getTakings() -> Float{
         return Float(self.takings) ?? 0
     }
 }
